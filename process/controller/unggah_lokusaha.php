...
$destination_file = '../dist/img/abc/';
	...
	if(isset($_FILES['fupload_lokusaha'])){
	

		// Ambil beberapa informasi tentang si foto
		$lokasi_file = $_FILES['fupload_lokusaha']['tmp_name'];
		$tipe_file = $_FILES['fupload_lokusaha']['type'];
		$nama_file = $_FILES['fupload_lokusaha']['name'];
		$file_size = $_FILES['fupload_lokusaha']['size'];
		...
		if (!empty($lokasi_file)){
			...
			if(in_array($file_ext,$expensions)== false){
				echo "<script>window.alert('Upload dokumen lokasi usaha gagal, pastikan file yang di upload bertipe *.JPG, *.PNG, *.GIF');
					history.go(-1);
					</script>
					";
			...
			/*----------------------------------------------------------------------------------*/
				...
				/* versi PDO */
				$data_gbr = mysql_query("SELECT lokusaha FROM dokumen 
											WHERE id_user = '" . $_SESSION['id_user'] . "'");
				$countData = mysql_num_rows($data_gbr);
				$r = mysql_fetch_assoc($data_gbr);
					
				// Upload file terlebih dahulu
  				move_uploaded_file($_FILES["fupload_lokusaha"]["tmp_name"], $destination_file . $nama_file_unik);

  				// cek if row kk by no regis sekarang sdah ada, jika iya update data dan delete photo sebelumnya
				if ($countData > 0) {

					// hapus file sebelumnya
					@unlink('../dist/img/abc/'.$r['lokusaha']); // delete file
				
					$sql = mysql_query("UPDATE dokumen SET lokusaha = '$nama_file_unik' WHERE id_user = '" . $_SESSION['id_user'] . "'");
					...
					<script>
						alert('Data dokumen foto Lokasi Usaha telah diupdate.');
						history.go(-2);
					</script>";
					...
					<script>
						alert('Data dokumen foto Lokasi Usaha telah ditambahkan.');
						history.go(-2);
					</script>";
				}

	...
	if(isset($_FILES['fupload_kk'])){
		...
		if (!empty($lokasi_file)){
			...
			/*----------------------------------------------------------------------------------*/
				...
				/* versi PDO */
				$data_gbr = mysql_query("SELECT kk FROM dokumen 
											WHERE id_user = '" . $_SESSION['id_user'] . "'");
				$countData = mysql_num_rows($data_gbr);
				$r = mysql_fetch_assoc($data_gbr);
					
				// Upload file terlebih dahulu
  				move_uploaded_file($_FILES["fupload_kk"]["tmp_name"], $destination_file . $nama_file_unik);

  				// cek if row kk by no regis sekarang sdah ada, jika iya update data dan delete photo sebelumnya
				if ($countData > 0) {

					// hapus file sebelumnya
					@unlink('../dist/img/abc/'.$r['kk']); // delete file
				
					$sql = mysql_query("UPDATE dokumen SET kk = '$nama_file_unik' WHERE id_user = '" . $_SESSION['id_user'] . "'");
					...
					$sql = mysql_query("INSERT INTO dokumen(id_user,kk,tgl_upload) VALUES ('" . $_SESSION['id_user'] . "','$nama_file_unik','".date("Y-m-d")."')");

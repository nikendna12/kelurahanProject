	...
	if(isset($_FILES['fupload_ktp'])){
		...
		if (!empty($lokasi_file)){
			...
			if(in_array($file_ext,$expensions)== false){
				echo "<script>window.alert('Upload dokumen KTP gagal, pastikan file yang di upload bertipe *.JPG, *.PNG, *.GIF');
					history.go(-1);
					</script>
					";
			...
			/*----------------------------------------------------------------------------------*/
				...
				/* versi PDO */
				$data_gbr = mysql_query("SELECT ktp FROM dokumen 
				...
				if ($countData > 0) {
					...
					@unlink('../dist/img/abc/'.$r['ktp']); // delete file
				
					$sql = mysql_query("UPDATE dokumen SET ktp = '$nama_file_unik' WHERE id_user = '" . $_SESSION['id_user'] . "'");
					...
					<script>
						alert('Data dokumen KTP telah diupdate.');
						history.go(-2);
					</script>";

				}
				else {
					$sql = mysql_query("INSERT INTO dokumen(id_user,ktp,tgl_upload) VALUES ('" . $_SESSION['id_user'] . "','$nama_file_unik','".date("Y-m-d")."')");
					...
					<script>
						alert('Data dokumen KTP telah ditambahkan.');
						history.go(-2);
					</script>";
				}

...
$destination_file = '../dist/img/abc/';
	...
	if(isset($_FILES['fupload_supernyataan'])){
	

		// Ambil beberapa informasi tentang si foto
		$lokasi_file = $_FILES['fupload_supernyataan']['tmp_name'];
		$tipe_file = $_FILES['fupload_supernyataan']['type'];
		$nama_file = $_FILES['fupload_supernyataan']['name'];
		$file_size = $_FILES['fupload_supernyataan']['size'];
		...
		if (!empty($lokasi_file)){
			...
			if(in_array($file_ext,$expensions)== false){
				echo "<script>window.alert('Upload dokumen Surat Pernyataan gagal, pastikan file yang di upload bertipe *.JPG, *.PNG, *.GIF');
					history.go(-1);
					</script>
					";
			...
			/*----------------------------------------------------------------------------------*/
				...
				/* versi PDO */
				$data_gbr = mysql_query("SELECT supernyataan FROM dokumen 
											WHERE id_user = '" . $_SESSION['id_user'] . "'");
				$countData = mysql_num_rows($data_gbr);
				$r = mysql_fetch_assoc($data_gbr);
					
				// Upload file terlebih dahulu
  				move_uploaded_file($_FILES["fupload_supernyataan"]["tmp_name"], $destination_file . $nama_file_unik);

  				// cek if row kk by no regis sekarang sdah ada, jika iya update data dan delete photo sebelumnya
				if ($countData > 0) {

					// hapus file sebelumnya
					@unlink('../dist/img/abc/'.$r['supernyataan']); // delete file
				
					$sql = mysql_query("UPDATE dokumen SET supernyataan = '$nama_file_unik' WHERE id_user = '" . $_SESSION['id_user'] . "'");
					...
					<script>
						alert('Data dokumen foto Surat Pengantar telah diupdate.');
						history.go(-2);
					</script>";

				}
				else {
					$sql = mysql_query("INSERT INTO dokumen(id_user,lokusaha,tgl_upload) VALUES ('" . $_SESSION['id_user'] . "','$nama_file_unik','".date("Y-m-d")."')");
					...
					<script>
						alert('Data dokumen foto Surat Pengantar telah ditambahkan.');
						history.go(-2);
					</script>";
				}
